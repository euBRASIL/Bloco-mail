export const idlFactory = ({ IDL }) => {
  ...
  const ApiError = IDL.Variant({
    'AliasHasBeenTaken' : IDL.Null,
    'ZeroAddress' : IDL.Null,
    'AliasFormatFail' : IDL.Text,
  ...
  const erc721_token = IDL.Service({
    ...
    'query_bind_alias_by_pid' : IDL.Func(
        [IDL.Principal],
        [IDL.Opt(IDL.Text)],
        ['query'],
      ),
    'query_bind_nft_by_pid' : IDL.Func(
        [IDL.Principal],
        [IDL.Opt(TokenMetadata)],
        [],
      ),
    'query_nft_by_name' : IDL.Func(
        [IDL.Text],
        [IDL.Variant({ 'Ok' : IDL.Opt(IDL.Text), 'Err' : ApiError })],
        [],
      ),
